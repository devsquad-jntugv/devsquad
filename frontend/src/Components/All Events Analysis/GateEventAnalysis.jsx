    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                ...
                const GateClub = response.data.filter(event => event.clubName === 'Gate-Exam');
                setEvents(GateClub);
            } catch (error) {
                ...
                {events.length>0?events.map((item,index)=>{
                    return(
                        <div key={item._id} className='grid grid-cols-[1fr,1fr,1fr,1fr,1fr] sm:grid-cols-[2fr,1fr,1fr,1fr,1.1fr] lg:grid-cols-[2fr,1fr,1fr,1fr,1.3fr] items-center justify-between py-2 lg:gap-3 sm:gap-2'>
                            ...
                            <button className='py-1 px-1 sm:py-2 sm:px-2 font-medium text-sm rounded-sm sm:rounded-md bg-[#00df9a] sm:ml-5 hover:cursor-pointer' on onClick={()=>{navigate('/broadcast')}}>Know More</button>
                        </div>
                    )
...
export default GateEventAnalysis